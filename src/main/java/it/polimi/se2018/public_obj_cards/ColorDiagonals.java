        ...
        public int calculateScore(WindowPatternCard w) {
            ...
            for (int row = 0; row < 3; row++) { //eccetto l'ultima riga
                for (int col = 0; col < 5; col++) {
                    ...
                    if (w.getSchema().get(row * 5 + col).getAssociatedDie().isPresent()) {
                        currentColor = w.getSchema().get(row * 5 + col).getAssociatedDie().get().getColor();
                        if (col > 0) { //checking sud-ovest cell die color
                            if (w.getSchema().get((row + 1) * 5 + (col - 1)).getAssociatedDie().isPresent()) {
                                if (w.getSchema().get((row + 1) * 5 + (col - 1)).getAssociatedDie().get().getColor() == currentColor) {
                                    countScore++;
                                }
                            }
                        }
